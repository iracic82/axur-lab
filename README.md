...
| EASM | Empty until a **seed** is added. The preload registers example.com **with its monitoring policy** and queues discovery. Measured 2026-09-15: assets appeared 51 minutes after one policy-enabled seed and 14 hours after another, and only the policy-enabled seed produced an exposure (DMARC misconfiguration, severity 55, roughly a day later); seeds registered without a policy got assets within a day at best and never an exposure. A lab session therefore still usually shows 0 assets, and exposures need a tenant that is at least a day old (pre-warmed pool) | preload (tested), Axur-scheduled |
...
| Lookalike domains: ticket opened automatically, no setup; Quarantine re-checked daily at 06:00 by the analyst's choice | [Smart Monitoring of Similar Domains](https://help.axur.com/en/articles/11566976-smart-monitoring-of-similar-domains), [Actions within a ticket](https://help.axur.com/en/articles/9005224-actions-within-a-ticket) |
